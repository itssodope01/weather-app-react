    ...
    async function fetchUserLocation() {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
          ...
          if (cachedCoords && haversineDistance(cachedCoords, { latitude, longitude }) < POSITION_CHANGE_THRESHOLD) {
            ...
            try {
              ...
              if (error.message === 'API Limit exceeded') {
                setApiError('API Limit exceeded. Please try again later.');
              } else if (error.message === 'Requested data not found') {
                setApiError('Requested data not found.');
              } else {
                setApiError('Error fetching location data.');
              }
              setPlace(DEFAULT_PLACE);
            }
          }
          ...
          setApiError('Geolocation error. Please enable location services and try again.');
          setPlace(DEFAULT_PLACE);
          setLocationDetermined(true);
        }, {
          timeout: 20000,
